...
#region Methods
    ...
    private void DoSpawningDishOrder()
    {
        ...
        for (int i = 0; i < _prefabSpawnLocations.Length; i++)
        {
            ...
            if (!_prefabSpawnLocations[i].GetComponent<CustomerSeat>().HasCustomer) 
            {
                ...
                _prefabSpawnLocations[i].gameObject.GetComponent<CustomerSeat>().HasCustomer = true;
    ...
    public bool IsEmptySpawnLocation()
    {
        ...
        for (int i = 0; i < _prefabSpawnLocations.Length; i++)
        {
            if (_prefabSpawnLocations[i].gameObject.GetComponent<CustomerSeat>().HasCustomer == false)
            {
